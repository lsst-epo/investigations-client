...
import { getAuthCookies, setAuthCookies } from "@/components/auth/serverHelpers";
...
async function conditionallySetCookiesRevalidatePath(authData:AuthFragmentFragment, pathToRevalidate:string) {
  if (authData) {
...
export async function POST(request: NextRequest) {
    ...
    if (!token || !path ) {
        return NextResponse.json( {error: 'Missing information needed to sign in'}, {status: 400});
    }
    ...
    if (craftRefreshToken) res.cookies.set("craftRefreshToken", craftRefreshToken);
